...
mol = gto.M(atom="7-inputs/methanal.xyz",unit="Angstrom",basis="pcseg-0",charge=0)
...
print("QM/MM calculations took:", timer()-start,"s")
...
for x in range(0,3):
    ...
    forces_resp_an.append(forces_resp_qm[x,:,J,x])


grad_resp = resp.nuc_grad_method()
f_vac = -grad_resp.kernel(state=state)

print("SCF Force sum test:")
print(np.einsum('xnAy->xny',forces_qm)+np.einsum('xnAy->xny',forces_mm))

print("Force resp sum test:")
print(np.einsum('xnAy->xny',forces_resp_qm)+np.einsum('xnAy->xny',forces_resp_mm))
